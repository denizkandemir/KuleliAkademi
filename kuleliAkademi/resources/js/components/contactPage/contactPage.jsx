...
const contactCards = [
  {
    ...
    title: 'Instagram',
    type: 'social',
    items: [{ label: `@${contactConfig.social.instagram.account}`, link: contactConfig.social.instagram.url }],
    icon: <InstagramIcon />,
  },
  {
    id: 2,
    title: 'Facebook',
    type: 'social',
    items: [{ label: contactConfig.social.facebook.account, link: contactConfig.social.facebook.url }],
    icon: <FacebookIcon />,
  },
  {
    id: 3,
    title: 'E-Posta',
    type: 'email',
    items: [{ label: contactConfig.email, link: `mailto:${contactConfig.email}` }],
    icon: mailIcon,
  },
  {
    id: 4,
...
export default function ContactPage({ formAction = '/contact' }) {
  const { props } = usePage();
  ...
  return (
    <div className="contact-container">
      <Banner text="İletişim" img={contactBanner} responsiveImg={contactBanner} location="İletişim" />

      <div className="contact-content-container fade-in">
        {props.flash?.success ? <p className="contact-success-banner">{props.flash.success}</p> : null}
        ...
        <div className="contact-cards-content-container">
                  ...
                  <div className="card-texts-container">
                    ...
                    {card.type !== 'whatsapp' ? (
                      <div className="card-item">
                        <a className="card-link" href={card.items[0].link} {...contactConfig.linkAttrs.external}>
                      ...
                      <div className="card-whatsapp-items">
                        {card.items.map((item, index) => (
                          <a key={index} className="whatsapp-item-link" href={item.link} {...contactConfig.linkAttrs.external}>
          ...
          <section className="contact-form-card">
            <div className="contact-form-copy">
              <p className="contact-form-kicker">Mesaj Gönder</p>
              <h2>Size geri dönmemiz için kısa bir not bırakın.</h2>
            </div>
            ...
            <form className="contact-form" onSubmit={submit}>
              <div className="contact-form-grid">
                <label className="contact-field">
                  <span>Ad Soyad</span>
                  <input type="text" value={data.name} onChange={(event) => setData('name', event.target.value)} required />
                  {errors.name ? <small className="contact-field-error">{errors.name}</small> : null}
                </label>

                <label className="contact-field">
                  <span>E-posta</span>
                  <input type="email" value={data.email} onChange={(event) => setData('email', event.target.value)} required />
                  {errors.email ? <small className="contact-field-error">{errors.email}</small> : null}
                </label>
              </div>
              ...
              <div className="contact-form-grid">
                <label className="contact-field">
                  ...
                  {errors.phone ? <small className="contact-field-error">{errors.phone}</small> : null}
                </label>

                <div className="contact-field contact-field--note">
                  <span>Not</span>
                  <p>Başvurular ve iletişim mesajları tek bir panelden takip edilir.</p>
                </div>
              </div>

              <label className="contact-field contact-field--textarea">
                <span>Mesaj</span>
                <textarea value={data.message} onChange={(event) => setData('message', event.target.value)} rows="5" required />
                {errors.message ? <small className="contact-field-error">{errors.message}</small> : null}
              </label>

              <div className="contact-form-actions">
                <button type="submit" className="contact-submit-button" disabled={processing}>
                  {processing ? 'Gönderiliyor...' : 'Mesajı Gönder'}
                </button>
              </div>
            </form>
          </section>
